...
export default function Footer() {
  ...
  const { width, isMobile, isTablet, isDesktop, isNanoScreen, isUltrawide } = useWindowSize();
  const isExtraSmall = width <= breakpoints.xs;

  // load Poppins font for footer
  useEffect(() => {
  ...
  const getContainerPadding = () => {
    if (isNanoScreen) return 'px-1 py-1';
    if (isExtraSmall) return 'px-4 py-3';
    if (isMobile) return 'px-4 py-4';
    if (isTablet) return 'px-6 py-4';
    return 'px-8 py-4';
  };

  // Calculate logo size based on screen
  const getLogoSize = () => {
    if (isNanoScreen) return { width: 40, height: 14 };
    if (isExtraSmall) return { width: 90, height: 31 };
    if (isUltrawide) return { width: 150, height: 52 };
  ...
  // Maintain the original structure but with responsive content
  return (
    <section className="w-full bg-white shadow-sm flex justify-center">
      <div className="w-full flex justify-center">
        ...
        >
          {isNanoScreen ? (
            // Special layout for nano screens
            <div className="flex flex-col items-center justify-center gap-1 py-1 w-full">
              ...
              {/* Privacy/Cookie/Copyright - stacked */}
              <div className={`flex flex-col items-center font-['Poppins'] ${getFontSize()} leading-tight gap-1`}>
                <button
            ...
            // Default row layout for normal screens - with exact 1440px width and 100px height
            <div className="flex flex-row items-center justify-between w-[1440px] h-[100px] mx-auto">
              {/* Logo - Left */}
              ...
              {/* Privacy policy / Cookie / Copyright - Center */}
              <div className={`flex justify-center items-center font-['Poppins'] ${getFontSize()} leading-6 footer-policy-links`}>
                <button
                  onClick={handlePrivacyPolicyClick}
                  className="whitespace-nowrap hover:opacity-80"
                >
              ...
              {/* Social icons - Right */}
              <div className="flex justify-end items-center social-media-icons">
                <Link
                  href="https://www.youtube.com/@arfve"
                  target="_blank"
                  rel="noopener noreferrer"
