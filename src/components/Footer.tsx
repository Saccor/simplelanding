...
const Footer: React.FC = () => {
  ...
  const logoWidth = mounted ? (isExtraSmall ? 70 : isTablet ? 80 : 88) : 88;
  const logoHeight = mounted ? (isExtraSmall ? 21 : isTablet ? 24 : 27) : 27;
  const textSize = mounted ? (isExtraSmall ? '11px' : isTablet ? '12px' : '14px') : '14px';
  const iconScale = mounted ? (isExtraSmall ? 0.8 : isTablet ? 0.9 : 1) : 1;
  const iconGap = mounted ? (isExtraSmall ? '12px' : isTablet ? '15px' : '18px') : '18px';
  ...
  return (
    <footer className="w-full bg-white flex justify-center">
      <div 
        className="w-full max-w-[1440px] flex items-center justify-between bg-white"
        style={{
      ...
      >
        {/* Left section - Logo */}
        <div className="flex-none" style={{ flex: isMobile ? 'unset' : '0 0 auto' }}>
          <Image 
        ...
        <div className="flex-none flex items-center" style={{ flex: isMobile ? 'unset' : '0 0 auto' }}>
          <div 
            className="flex items-center"
            style={{
              gap: iconGap,
